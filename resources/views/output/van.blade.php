...
<div class="logbook-container">
    <header>
        ...
        <h2>COOLING VAN LOGBOOK</h2>
        <div class="header-details">
            <p><strong>Date:</strong> <span>{{$record->created_at}}</span></p>
            <p><strong>Driver's Name:</strong> <span>{{$record->driver_name}}</span></p>
    ...
    <div class="logbook-table-section">
        <table class="print-table">
            ...
            <tr>
                ...
                <th class="tb-hd">Departure Time</th>
                <th class="tb-hd">Amount Products Carried</th>
                <th class="tb-hd">Delivery Location(s)</th>
                <th class="tb-hd">Return Time</th>
                <th class="tb-hd">Fuel Level (In/Out)</th>
                <th class="tb-hd">Remarks</th>
            </tr>
            </thead>
            <tbody>
            @foreach($record->vanItems as $item)
                <tr>
                    <td class="tdd">{{ $loop->iteration }}</td>
                    <td class="tdd">{{ $item->departure_time }}</td>
                    <td class="tdd">{{ $item->amount_products_carried }}</td>
                    <td class="tdd">{{ $item->delivery_location }}</td>
                    <td class="tdd">{{ $item->return_time }}</td>
                    <td class="tdd">{{ $item->fuel_level }}</td>
